// components/pages/GenresPageClient.tsx
'use client';
...
export default function GenresPageClient({
  ...
  const selectGenre = (genre: any) => {
    ...
    setCurrentPage(1);
    setMixedContent([]);
    fetchInitialContent(genre);
  };

  const fetchInitialContent = async (genre: any) => {
    setIsLoading(true);
    try {
      const data = await fetchMixedContent(genre, 1);
  ...
  const fetchMixedContent = async (genre: any, page: number) => {
    if (!genre) return [];
  ...
  const loadMoreContent = async () => {
    ...
    try {
      const data = await fetchMixedContent(selectedGenre, nextPage);
  ...
  const handleSortChange = (newSort: string) => {
    setSortBy(newSort);
    if (selectedGenre) {
      fetchInitialContent(selectedGenre);
    }
  };
