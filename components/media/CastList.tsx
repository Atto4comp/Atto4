...
export default function CastList({ cast }: CastListProps) {
  ...
  const visibleCast = cast.slice(0, 20); // Show a few more
  ...
  return (
    <div className="w-full py-8 relative">
      ...
      {/* 
         Fix: Added 'w-full' and proper masking.
         The -mx-4 md:mx-0 pulls it to edges on mobile but keeps it contained on desktop.
      */}
      <div className="relative w-full group">
        <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-4 px-4 md:px-0 snap-x snap-mandatory w-full">
          {visibleCast.map((person) => {
            const imageUrl = buildProfileImage(person.profile_path);
            return (
              <div 
                key={person.id} 
                className="flex flex-col items-center w-[90px] md:w-[100px] flex-shrink-0 snap-start cursor-pointer group/actor"
              >
                <div className="relative w-[80px] h-[80px] md:w-[90px] md:h-[90px] rounded-full overflow-hidden border-2 border-white/10 bg-gray-800 shadow-lg transition-all duration-300 group-hover/actor:border-white/40 group-hover/actor:shadow-xl mb-3">
                  {imageUrl ? (
                    <Image
                      src={imageUrl}
                      alt={person.name}
        ...
        <div className="absolute inset-y-0 right-0 w-24 bg-gradient-to-l from-[#050505] to-transparent pointer-events-none hidden md:block" />
      </div>
    </div>
  );
}
