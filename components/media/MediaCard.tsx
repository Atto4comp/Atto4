      ...
      <Link href={`/${mediaType}/${media.id}`} className="block">
        ...
        <div className="relative aspect-video rounded-xl overflow-hidden bg-gray-900 border border-white/5 transition-all duration-300 group-hover:border-white/20 group-hover:shadow-[0_0_30px_rgba(255,255,255,0.1)]">
          ...
          {media.vote_average > 0 && (
            <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-md border border-white/10 px-1.5 py-0.5 rounded-md flex items-center gap-1 z-20">
              <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
              <span className="text-[10px] font-bold text-white">{media.vote_average.toFixed(1)}</span>
            </div>
          )}

          {/* Hover Actions - z-index 20 to stay above gradient */}
          <div className={`absolute inset-0 flex items-center justify-center gap-3 transition-opacity duration-300 z-20 ${hover ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <div className="absolute inset-0 bg-black/50 backdrop-blur-[2px] -z-10" />
            
            <Link
              href={`/watch/${mediaType}/${media.id}`}
              className="w-11 h-11 bg-white text-black rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow-lg"
