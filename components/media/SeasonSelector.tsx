...
export default function SeasonSelector({ seasons, currentSeason, onSelect }: SeasonSelectorProps) {
  ...
  return (
    <div className="relative z-[100]" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-64 bg-[#151515] border border-white/10 rounded-xl overflow-hidden shadow-2xl z-[100] animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="max-h-[300px] overflow-y-auto py-1 season-scrollbar">
            {seasons.map((season) => (
              <button
                key={season.id}
                onClick={() => {
