      ...
      <div className="min-h-screen bg-black pb-20">
        ...
        <div className="relative z-10 flex flex-col items-center px-6 pt-24">
          ...
          <div className="flex items-center gap-3 mb-8 w-full justify-center max-w-sm">
            ...
            <button onClick={toggleWatchlist} className={`w-12 h-12 rounded-xl flex items-center justify-center border transition-all active:scale-95 ${isInWatchlist ? 'bg-green-500 border-green-500 text-white' : 'bg-white/5 border-white/10 text-white'}`}>
              <Plus className={`w-5 h-5 ${isInWatchlist ? 'rotate-45' : ''}`} />
            </button>
            <button onClick={toggleLike} className={`w-12 h-12 rounded-xl flex items-center justify-center border transition-all active:scale-95 ${isLiked ? 'bg-red-500 border-red-500 text-white' : 'bg-white/5 border-white/10 text-white'}`}>
          ...
          <div className="w-full min-h-[200px] mb-8">
            {loadingEpisodes ? (
              <div className="flex gap-4 overflow-hidden px-2">
                {[1,2].map(i => <div key={i} className="w-[280px] aspect-video bg-white/5 rounded-xl animate-pulse flex-shrink-0" />)}
              </div>
            ) : (
  ...
  return (
    <div className="relative min-h-screen bg-black text-white pt-20">
      ...
      <div className="relative z-10 w-full max-w-[1600px] mx-auto px-8 md:px-16 pt-8 flex gap-12 items-start min-h-screen">
        ...
        {/* Info Area */}
        <div className="flex-1 pb-20 min-w-0"> {/* min-w-0 fixes flex child overflow */}
          ...
          <div className="flex items-center gap-4 mb-8 text-sm text-gray-300 font-medium flex-wrap">
            ...
            <span className="w-1 h-1 bg-gray-600 rounded-full" />
            <span className="bg-white/10 px-2.5 py-1 rounded-md text-xs font-bold">TV-MA</span>
            <span className="w-1 h-1 bg-gray-600 rounded-full" />
            <span>{tv.number_of_seasons} Seasons</span>
            <span className="w-1 h-1 bg-gray-600 rounded-full" />
            <span className="flex items-center gap-1 text-yellow-400 font-semibold"><Star className="w-4 h-4 fill-current" /> {tv.vote_average.toFixed(1)}</span>
          </div>
